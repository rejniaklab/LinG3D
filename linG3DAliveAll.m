function linG3DAliveAll(pathData,numClones,IsGradient,xmin,xmax,...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 view(3)
 hold on
 col=DefineColorPalette; Ncol=size(col,1);  % define color palette
 ...
 for cloneNum=0:numClones
   disp(['clone=',num2str(cloneNum),' of ',num2str(numClones)]);
    
   % identify all survived cells from a given clone 
   indLast=[]; Nlast=0;     
   for ii=1:length(cellID)    % all cells in the last file
   ...
   for ii=1:Numlast
     ...
     while (moNum>0)&&(hist(moNum,2)==cloneNum)  % all predecesor cells
       Nlast=Nlast+1;
       indLast(Nlast)=moNum;
       moNum=hist(moNum,3);
     end
   ...
   matrix_to_draw=zeros(1,6);  % [x1,t1,y1,x2,t2,y2] X-time-Y axes
   Nmatrix=0;
 ...
 title('final 3D traces of the survived clones')
 view(3)
 axis equal
 axis([xmin,xmax,tmin,tmax/timeStep,ymin,ymax])

 if toPrint==1
   print('-djpeg100','-r300',[pathData,'/tree_alive_combined.jpg'])
 end
